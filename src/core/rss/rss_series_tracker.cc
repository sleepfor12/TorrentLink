...
QString extractQuality(const QString& title) {
  static const QRegularExpression re(QStringLiteral("(2160p|1080p|720p|480p|4K|UHD)"),
...
std::optional<EpisodeInfo> RssSeriesTracker::parseTitle(const QString& title) {
  ...
  // Pattern 3: 第05集 / 第5话
  {
    static const QRegularExpression re(QStringLiteral("(.+?)\\s*第\\s*(\\d{1,4})\\s*[集话期回]"));
    const auto m = re.match(title);
    if (m.hasMatch()) {
      ep.series_name = m.captured(1).trimmed();
  ...
  {
    static const QRegularExpression re(QStringLiteral("(.+?)\\s*EP\\.?(\\d{1,4})"),
                                       QRegularExpression::CaseInsensitiveOption);
    const auto m = re.match(title);
    if (m.hasMatch()) {
      ep.series_name = m.captured(1).trimmed();
...
bool RssSeriesTracker::matchesSeries(const EpisodeInfo& ep, const SeriesSubscription& sub) {
  const QString epName = ep.series_name.toLower().trimmed();

  if (sub.name.toLower().trimmed() == epName)
    return true;

  for (const auto& alias : sub.aliases) {
    if (alias.toLower().trimmed() == epName)
      return true;
    if (epName.contains(alias.toLower().trimmed()))
      return true;
  }

  if (sub.name.toLower().trimmed().isEmpty())
    return false;
  return epName.contains(sub.name.toLower().trimmed());
}

bool RssSeriesTracker::isNewerEpisode(const EpisodeInfo& ep, const SeriesSubscription& sub) {

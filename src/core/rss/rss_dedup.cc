...
QString RssDedup::extractInfoHash(const QString& magnet) {
  ...
  static const QRegularExpression re32(QStringLiteral("xt=urn:btih:([A-Za-z2-7]{32})"));
  const auto m32 = re32.match(magnet);
  if (m32.hasMatch())
    return m32.captured(1).toLower();
...
void RssDedup::buildIndex(const std::vector<RssItem>& existing) {
  ...
  LOG_DEBUG(
      QStringLiteral("[rss.dedup] buildIndex existing=%1 ids=%2 guids=%3 links=%4 infohashes=%5")
          .arg(existing.size())
          .arg(known_ids_.size())
...
std::vector<RssItem> RssDedup::filterDuplicates(const std::vector<RssItem>& existing,
  ...
  LOG_INFO(
      QStringLiteral("[rss.dedup] filterDuplicates existing=%1 incoming=%2 unique=%3 dropped=%4")
          .arg(existing.size())
          .arg(incoming.size())
          .arg(unique.size())
...
int RssDedup::pruneHistory(std::vector<RssItem>& items, const HistoryPolicy& policy) {
  ...
  if (policy.max_age_days > 0) {
    ...
    items.erase(std::remove_if(items.begin(), items.end(),
                               [&](const RssItem& it) {
                                 return it.published_at.isValid() && it.published_at < cutoff &&
                                        !it.downloaded;
                               }),
                items.end());
  }
  ...
  if (policy.max_items > 0 && static_cast<int>(items.size()) > policy.max_items) {
    std::sort(items.begin(), items.end(), [](const RssItem& a, const RssItem& b) {
  ...
  if (removed > 0) {
    LOG_INFO(QStringLiteral(
                 "[rss.dedup] pruneHistory removed=%1 remaining=%2 max_items=%3 max_age_days=%4")
                 .arg(removed)
                 .arg(items.size())

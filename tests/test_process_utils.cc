...
TEST(ProcessUtils, BuildDetachedCommandParsesProgramAndArgs) {
  const auto cmd = buildDetachedCommand(QStringLiteral("mpv --no-video"), {QStringLiteral("a.mp4")});
...
TEST(ProcessUtils, ExecutableSuffixMatchesPlatformConvention) {
  ...
  EXPECT_EQ(withPlatformExecutableSuffix(QStringLiteral("python.exe")), QStringLiteral("python.exe"));
#else
